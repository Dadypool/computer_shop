...
async def delete_or_create(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Выберите тип действия", reply_markup=seller_kb.delete_or_create()
    )
...
async def enter_id(message: types.Message, state: FSMContext):
    ...
    if product_to_delete == None:
        ...
        await message.answer(
            ...
            reply_markup=seller_kb.yes_or_no(product_to_delete["id"]),
        )
...
async def delete_product(callback: types.CallbackQuery, state: FSMContext):
    ...
    if product.delete_product(int(id)):
        await callback.message.answer(
            "Товар успешно удален", reply_markup=seller_kb.sellermenu()
        )
        await state.set_state(sellerstate.menu)
    else:
        await callback.message.answer(
            "Ошибка удаления, повторите попытку", reply_markup=seller_kb.sellermenu()
        )
        await state.set_state(sellerstate.menu)
...
async def add_product(message: types.Message, state: FSMContext):
    ...
    if len(parsed) != 5:
        ...
        try:
            ...
            ):  # (int(id), str(name), int(price), str(category), str(manufacturer))
                await message.answer(
                    "Товар успешно добавлен", reply_markup=seller_kb.sellermenu()
                )
                await state.set_state(sellerstate.menu)
            else:
...
async def order_view(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.message.answer(
        "Выберите действие для заказа", reply_markup=seller_kb.order_action()
    )
...
async def approve(callback: types.CallbackQuery, state: FSMContext):
    ...
    if orders:
        await callback.message.answer(
            ...
            reply_markup=seller_kb.orders(orders, "apv:"),
        )
    else:
        await callback.message.answer(
            "Нет созданных заказов", reply_markup=seller_kb.sellermenu()
        )
...
async def ready(callback: types.CallbackQuery, state: FSMContext):
    ...
    if orders:
        await callback.message.answer(
            "Выберите заказ для выдачи", reply_markup=seller_kb.orders(orders, "giv:")
        )
    else:
        await callback.message.answer(
            "Нет подтвержденных заказов", reply_markup=seller_kb.sellermenu()
        )
...
async def approve_by_id(callback: types.CallbackQuery, state: FSMContext):
    ...
    if order.update_order_status(int(id), "confirmed"):  # order.update_order_status
        await callback.message.answer(
            "Заказ подтвержден", reply_markup=seller_kb.sellermenu()
        )
    else:
        await callback.message.answer(
            "Ошибка подтверждения, повторите попытку",
            reply_markup=seller_kb.sellermenu(),
        )
    await state.set_state(sellerstate.menu)
...
async def hardset(callback: types.CallbackQuery, state: FSMContext):
    ...
    if order.update_order_status(int(id), "closed"):  # order.update_order_status
        await callback.message.answer(
            "Заказ выдан", reply_markup=seller_kb.sellermenu()
        )
    else:
        await callback.message.answer(
            "Ошибка выдачи, повторите попытку", reply_markup=seller_kb.sellermenu()
        )
    await state.set_state(sellerstate.menu)
