...
async def register(message: types.Message, state: FSMContext):
    if user.create_user(
        {"id": message.from_user.id, "name": message.text, "rights": "user"}
    ):
        await state.set_state(userstate.menu)
        await message.answer(
            "Вы успешно зарегистрированы", reply_markup=user_kb.usermenu()
        )
    else:
...
)  # вывод подкаталога (выбранного типа товара)
async def catalog(callback: types.CallbackQuery, state: FSMContext):
    ...
    if products:
        await callback.message.answer(
            "Выберете продукт", reply_markup=user_kb.products(products)
        )
    else:
        await callback.message.answer(
            "Ошибка чтения каталога.", reply_markup=user_kb.usermenu()
        )
...
async def add(callback: types.CallbackQuery, state: FSMContext):
    ...
    if product_id:
        if product.update_add_product_to_cart(callback.from_user.id, product_id):
            await callback.message.edit_reply_markup(reply_markup=None)
            await callback.message.answer(
                "Товар успешно добавлен в корзину!", reply_markup=user_kb.usermenu()
            )
        else:
            await callback.message.edit_reply_markup(reply_markup=None)
            await callback.message.answer(
                "Ошибка добавления в корзину!", reply_markup=user_kb.usermenu()
            )
    else:
        await callback.message.edit_reply_markup(reply_markup=None)
        await callback.message.answer(
            "Такого продукта нет в наличии!", reply_markup=user_kb.usermenu()
        )
...
async def view_order(callback: types.CallbackQuery, state: FSMContext):
    ...
    if orders:
        await callback.message.edit_reply_markup(reply_markup=None)
        await callback.message.answer(
            "Выберите заказ для просмотра", reply_markup=user_kb.orders(orders)
        )
    else:
        await callback.message.edit_reply_markup(reply_markup=None)
        await callback.message.answer(
            "У вас нет заказов", reply_markup=user_kb.usermenu()
        )
...
async def order_list(callback: types.CallbackQuery, state: FSMContext):
    ...
    order_list = order.read_products_by_order_id(
        int(order_id)
...
async def basket(callback: types.CallbackQuery, state: FSMContext):
    ...
    if products:
        ...
        await callback.message.answer(
            f"Если хотите удалить какой-то товаи из корзины - выберете его.\nОбщая сумма тованов в корзине составляет {total_price}€",
            reply_markup=user_kb.cart(products),
        )
    else:
...
async def basket_remove(callback: types.CallbackQuery, state: FSMContext):
    ...
    if product.update_remove_product_from_cart(callback.from_user.id, product_id):
        await callback.message.edit_reply_markup(reply_markup=None)
        await callback.message.answer(
            "Товар успешно удален из корзины!", reply_markup=user_kb.usermenu()
        )
    else:
        await callback.message.edit_reply_markup(reply_markup=None)
        await callback.message.answer(
            "Ошибка удаления из корзины!", reply_markup=user_kb.usermenu()
        )
...
async def buy(callback: types.CallbackQuery, state: FSMContext):
    ...
    if order.create_order(callback.from_user.id):
        await callback.message.answer(
            "Заказ успешно офомлен!", reply_markup=user_kb.usermenu()
        )
    else:
        await callback.message.answer(
            "Ошибка оформления заказа!", reply_markup=user_kb.usermenu()
        )
